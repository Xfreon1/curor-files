...
class NowPlayingWidget(Static):
    ...
    def _fetch_smtc_worker(self) -> None:
        try:
            ...
            pos_set_at = info.get("position_set_at")  # real-time unix ts from SMTC
            if pos is not None:
                self._last_pos = pos
                if pos_set_at is not None:
                    # Anchor interpolation to when the app actually set the position
    ...
    def _redraw(self) -> None:
        ...
        if self._last_pos is not None and self._last_duration is not None:
            if self._is_playing:
                elapsed_since = time.monotonic() - self._last_pos_time
                pos = min(self._last_pos + elapsed_since, self._last_duration)
            else:
                pos = self._last_pos
            info["position"] = pos
            info["duration"] = self._last_duration
        self._draw(info, self._last_level, self._last_muted)
